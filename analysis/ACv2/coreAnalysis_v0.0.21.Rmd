...
tmp <- offBrand[, c("advisor0idDescription", "pid", "advisor0woa", "hasFeedback", 
                    "firstAdvisor")]
names(tmp)[1] <- "advisor"

df <- aggregate(advisor0woa ~ pid + hasFeedback + advisor + firstAdvisor,
                 tmp, mean, na.rm = T)
df$feedback <- factor(df$hasFeedback)
...
offBrand %>%
  mutate(feedback = if_else(hasFeedback, "Feedback", "No feedback"),
         feedback = factor(feedback, levels = c("No feedback", "Feedback"))) %>%
...
```{r}
tmp <- offBrand %>% 
  mutate(feedback = factor(hasFeedback),
         advisor = factor(advisor0idDescription),
         order = factor(firstAdvisor))
a <- anovaBF(advisor0woa ~ advisor + feedback + order + pid, 
...
marginalBF(a, list(
  c("feedback + advisor + feedback:advisor + order + pid",
    "feedback + advisor + order + pid")
  )
)
```
