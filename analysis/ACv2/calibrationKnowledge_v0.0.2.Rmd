...
tmp <- AdvisedTrial %>% 
  ...
  group_by(pid, advisor0idDescription, feedback, hybrid) %>%
  summarise(advisorInfluence = mean(advisor0influence)) 

tmp %>% 
  group_by(advisor0idDescription, feedback, hybrid) %>%
...
tmp %>% 
  mutate(advisor = paste0(advisor0idDescription, if_else(hybrid, '?', ''))) %>%
...
AdvisedTrial %>%
  ...
  mutate(advisor = if_else(advisor0name == '?', 
                           'Hybrid', 
                           as.character(advisor0idDescription))) %>%
...
AdvisedTrial %>% 
  filter(!feedback) %>%
  mutate(hybrid = advisor0name == '?') %>%
ggplot(aes(advisor0influence)) + 
  geom_histogram() +
  facet_grid(hybrid ~ advisor0idDescription, labeller = label_both)
