...
for (i in 1:length(grep("advisor[0-9+]idDescription", names(AdvisedTrial)))) {
  ...
  if (length(grep(paste0("advisor", i, "actualType"), names(AdvisedTrial)))) {
...
for (v in names(AdvisedTrial)[grepl("advisor1", names(AdvisedTrial))]) {
  suffix <- reFirstMatch("advisor1(\\S+)", v)
  for (a in unique(c(as.character(AdvisedTrial$advisor1idDescription),
                     as.character(AdvisedTrial$advisor2idDescription)))) {
    
    s <- paste0(a, ".", suffix)
    AdvisedTrial[, s] <- AdvisedTrial[, paste0("advisor1", suffix)]
    
    for (i in 1:length(grep("^advisor[0-9]+id$", names(AdvisedTrial)))) {
      if (length(grep(paste0("^advisor", i, suffix), names(AdvisedTrial)))) {
        AdvisedTrial[AdvisedTrial[paste0("advisor", 
                                         i, 
                                         "idDescription")] == a, s] <-
          AdvisedTrial[AdvisedTrial[paste0("advisor", i, "idDescription")] == a,
...
debrief.form$guessedManipulation <- T
