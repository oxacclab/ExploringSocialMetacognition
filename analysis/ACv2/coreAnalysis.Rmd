...
markerList <- list(thin = 3, medium = 9, wide = 27)
...
for (f in files) {
  ...
  if ("stimHTML" %in% names(tmp)) {
    ...
    tmp$stimHTML <- gsub("[\r\n]", "", tmp$stimHTML)
    tmp$stimHTML <- gsub("  ", "", tmp$stimHTML, fixed = T)
    tmp$stimHTML <- gsub("<[^>]*>([^<]*)[^>]*>", "\\1", tmp$stimHTML)
  }
...
for (p in unique(exclusions$pid)) {
  ...
  if (any(tmp$responseMarkerWidth != 3))
...
We can now check for participants who are outliers as a whole, and remove them. Then we can inspect the final exclusion reason list.
...
for (v in checkList) {
  tmp <- scale(PP[[v]])
  PP$excluded[abs(tmp) > zThresh] <- 
    ifelse(PP$excluded[abs(tmp) > zThresh] == F,
           v, paste(PP$excluded, v, collapse = ", "))
  
  if (any(abs(tmp) > zThresh))
    print(ggplot(PP, aes(x = "", y = !!ensym(v))) +
            geom_violin(alpha = .25, color = NA, fill = "grey75") + 
            geom_boxplot(outlier.shape = NA, fill = NA) + 
            geom_point(position = position_jitter(.33), alpha = .5) +
            labs(x = "participants"))
...
ggplot(PP[!is.na(PP$responseMarker), ], 
  ...
  geom_point(alpha = .5, aes(colour = pid), position = position_jitter(.33)) +
...
First we can look at the response time - the difference between the time the response is opened and the time the response is received.  
...
**Correct Agree** | The advisor gives advice which is both correct and agreeing | Correct
...
for (f in unique(AdvisedTrial$hasFeedback)) {
  ...
  print(prop2str(tmp, precision = 3))
}
...
Advisors are supposed to differ in their accuracy. This is expected to hold true whether accuracy is cast in terms of %correct or mean error. 
...
ggplot(tmp, aes(x = advisor, y = value, colour = pid)) +
  ...
  geom_point(position = position_jitter(.33)) +
...
AdvisedTrial$estimateLeftChange <- AdvisedTrial$responseEstimateLeftFinal -
  AdvisedTrial$responseEstimateLeft
