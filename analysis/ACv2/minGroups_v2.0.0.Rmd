...
This study version uses a more salient manipulation: participants have to click on a message stating whether the advisor belongs to their group or the other group, and the participant's group is displayed continually alongside the advisor information.
...
ggplot(tmp, aes(x = sameGroup, y = rating)) +
  ...
  scale_color_discrete(guide = 'none') +
  facet_wrap(condition ~ Q, labeller = label_both)
...
tmp[['woa']] <- AdvisedTrial %>% 
  ...
  group_by(pid, advisor0idDescription, condition) %>%
...
tmp %>% ggplot(aes(x = inGroupWoaPref, y = inGroupTrustPref)) +
  ...
  geom_point(aes(colour = condition > 2)) +
  geom_smooth(aes(colour = condition > 2), 
