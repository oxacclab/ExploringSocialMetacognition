...
for (v in names(AdvisedTrial)[grepl("advisor0", names(AdvisedTrial))]) {
  ...
  for (a in advisorNames) {
    ...
    s <- paste0(a, ".", suffix)
    if (typeof(pull(AdvisedTrial, paste0('advisor0', suffix))) == 'character')
      AdvisedTrial[, s] <- NA_character_
    else if (typeof(pull(AdvisedTrial, paste0('advisor0', suffix))) == 'double')
      AdvisedTrial[, s] <- NA_real_
    else if (typeof(pull(AdvisedTrial, paste0('advisor0', suffix))) == 'integer')
      AdvisedTrial[, s] <- NA_integer_
    else if (typeof(pull(AdvisedTrial, paste0('advisor0', suffix))) == 'list')
      AdvisedTrial[, s] <- NA_complex_
    else
      AdvisedTrial[, s] <- NA
...
i <- min(AdvisedTrial$number)

AdvisedTrial <- AdvisedTrial %>% 
  nest(d = -pid) %>%
  mutate(firstAdvisor = map_chr(d, ~ filter(., number == min(number)) %>% pull(advisor0idDescription))) %>%
  unnest(cols = d)
...
tmp <- aggregate(advisor0woa ~
                 ...
                 block2, mean, na.rm = T)
tmp$hasFeedback <- ifelse(tmp$hasFeedback, 'Feedback', 'No feedback')
tmp <- tmp %>% mutate_if(is.character, factor) %>% as_data_frame()
