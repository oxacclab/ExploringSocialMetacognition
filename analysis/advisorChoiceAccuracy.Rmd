...
for(pid in unique(participants$pid)) {
  ...
  df.xii.i.i <- rbind(df.xii.i.i, 
                      data.frame(pid,
                                 ...
                                 InfluenceDiff=(mean(tmp.2$influence[tmp.2$adviceType==adviceTypes$HighAcc]) -
                                                  mean(tmp.2$influence[tmp.2$adviceType==adviceTypes$LowAcc]))))
}
...
```{r echo = F}
gg.xii.i.i <- ggplot(df.xii.i.i, aes(y = HighAccPref, x = InfluenceDiff)) +
  geom_hline(linetype = "dashed", color = "black", yintercept = .5, size = 1) +
...
df.xii.i.i.2 <- df.xii.i.i[abs(scale(df.xii.i.i$InfluenceDiff))<3 & 
                             abs(scale(df.xii.i.i$HighAccPref))<3, ]

cor.test(df.xii.i.i.2$HighAccPref, df.xii.i.i.2$InfluenceDiff)
correlationBF(df.xii.i.i.2$HighAccPref, df.xii.i.i.2$InfluenceDiff)

gg.xii.i.i.2 <- ggplot(df.xii.i.i.2, 
                       aes(y = HighAccPref, x = InfluenceDiff)) +
  geom_hline(linetype = "dashed", color = "black", yintercept = .5, size = 1) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0, fill = 'grey', alpha = .5) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, fill = 'grey', alpha = .5) +
  geom_smooth(method = 'lm', fill = 'lightblue', alpha = 0.5) +
