...
if(length(pairs) == 0)
  cat("*No advisor pairs found for analysis of forced trials.*")
...
```{r}
print("TODO: Fix this chunk not working in knitted output.")
...
Of critical importance is the difference in pick rate and influence made by the advisor sharing a group with the participant. 

## Pick rate

We hypothesise that the participant will preferentially pick the advisor assigned to their group. 
...
For a more detailed analysis, we look at the interactions between advice profile and group membership. The experimental design is balanced with regard to this: all participants see one advisor of each advice profile who shares their group, and one who does not. This would count as exploration so if included should be below.

## Influence

We can add a factor to the influence analyses which holds whether the group matches the participant's.
