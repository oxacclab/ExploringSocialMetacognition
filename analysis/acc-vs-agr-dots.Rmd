...
ggplot(df.choice, aes(x = pChooseAcc, y = feedbackCondition)) +
  ...
  geom_point(position = position_jitter(0, .05), alpha = .5,
             aes(y = as.numeric(factor(feedbackCondition)) - .05))
```

## Summary {.summary}

Participants do not seem to have learned about the differences in the advisors during the training phase, even with feedback. It may be that the current design, in which participants are trained on both advisors mixed together over two training blocks, is more difficult for participants to process than other designs we have used where participants are given an entire block of one advisor, then an entire block of the other. It may well be worth conducting a version of this experiment where advisors are introduced serially rather than mixed together.
