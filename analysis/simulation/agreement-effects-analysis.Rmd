...
We should explore whether the derivatives, as well as the asymptotes, are functions of the bias strength, and especially whether the relationship between them changes as so. 
...
mdlsConf <- runSimulations(params, seed = 20191127)
mdlsNoConf <- runSimulations(params %>% mutate(conf = F), seed = 20191127)
...
for (m in c(mdlsConf, mdlsNoConf)) {
  ...
  coefs <- rbind(coefs, 
                 tmp %>% mutate(conf = m$parameters$conf, 
...
The higher the mean bias, the higher faster the correlation reaches its plateau, both with and without confidence weighting. Increased mean bias also increases the level of the correlation at which the plateau occurs. The effect on the temporal evolution appears to be fairly similar whether or not confidence weighting is included. 
...
!TODO[Not an obvious pattern - should run this a few times and investigate.]
