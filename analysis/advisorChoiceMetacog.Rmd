...
for(pid in unique(participants$pid)) {
  ...
  df.xii.i.i <- rbind(df.xii.i.i, data.frame(pid,
                                             ...
                                             InfluenceDiff=(sum(tmp$influence[tmp$adviceType==adviceTypes$AiC]) -
                                                              sum(tmp$influence[tmp$adviceType==adviceTypes$AiU]))))
}
...
```{r echo = F}
gg.xii.i.i <- ggplot(df.xii.i.i, aes(x = AiCPref, y = InfluenceDiff)) +
  geom_vline(linetype = "dashed", color = "black", xintercept = .5, size = 1) +
  geom_smooth(method = 'lm') +
  geom_point(aes(colour = as.factor(pid))) +
  labs(title = 'Correlation between influence difference and preference for Agree-in-confidence advisor') +
  style.long
gg.xii.i.i
```
