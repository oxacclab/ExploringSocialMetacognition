---
title: "Analysis script for AdvisorChoice (Metacognitive-contingent advisors)"
...
#   1.i) Load data ####
print('Load data')
folderName <- "G:\\Documents\\University\\Google Drive\\Project Documents\\AdvisorChoice\\results\\MetaCog\\2018-09-18 2c Fixed\\processed"
...
tmp <- aggregate(adviceType ~ pid, 
                 data = trials[trials$type==trialTypes$choice 
                               ...
                               & trials$initialAnswer == trials$correctAnswer, ],
                 FUN = function(x)sum(x==adviceTypes$AiC)/length(x))
md.ttest(tmp$adviceType, mu=0.5)
```
...
If so, we should see that participants preferentially pick the AiC 
advisor when their initial confidence is high, and AiU when
...
tB.ix.i <- ttestBF(tmp$adviceType[tmp$confidenceCategory==confidenceCategories$low],
        tmp$adviceType[tmp$confidenceCategory==confidenceCategories$high],
        paired = T)
print('Choice proportion AiC in low- vs high-confidence trials')
...
print(tB.ix.i)
print(paste0('Evidence strength for differential AiC picking strategy by confidence: BF=', 
...
for(pid in unique(participants$pid)) {
  ...
  df.xii.i.i <- rbind(df.xii.i.i, 
                      data.frame(pid,
                                 AiCPref=mean(tmp$adviceType==adviceTypes$AiC),
                                 InfluenceDiff=(mean(tmp.2$influence[tmp.2$adviceType==adviceTypes$AiC]) -
                                                  mean(tmp.2$influence[tmp.2$adviceType==adviceTypes$AiU]))))
}
cor.test(df.xii.i.i$AiCPref, df.xii.i.i$InfluenceDiff)
correlationBF(df.xii.i.i$AiCPref, df.xii.i.i$InfluenceDiff)
```

```{r echo = F}
gg.xii.i.i <- ggplot(df.xii.i.i, aes(y = AiCPref, x = InfluenceDiff)) +
  geom_hline(linetype = "dashed", color = "black", yintercept = .5, size = 1) +
...
df.xii.i.i.2 <- df.xii.i.i[abs(scale(df.xii.i.i$InfluenceDiff))<3 & 
                             abs(scale(df.xii.i.i$AiCPref))<3, ]

cor.test(df.xii.i.i.2$AiCPref, df.xii.i.i.2$InfluenceDiff)
correlationBF(df.xii.i.i.2$AiCPref, df.xii.i.i.2$InfluenceDiff)

gg.xii.i.i.2 <- ggplot(df.xii.i.i.2, 
                       aes(y = AiCPref, x = InfluenceDiff)) +
  geom_hline(linetype = "dashed", color = "black", yintercept = .5, size = 1) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0, fill = 'grey', alpha = .5) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, fill = 'grey', alpha = .5) +
  geom_smooth(method = 'lm', fill = 'lightblue', alpha = 0.5) +
...
for(pid in unique(participants$pid)) {
  for(block in unique(trials$block[trials$type==trialTypes$choice])) {
    ...
    aid <- ifelse(mean(tmp$adviceType==adviceTypes$AiC)>.5, adviceTypes$AiC, adviceTypes$AiU)
...
gg.xii.i.ii <- ggplot(df.xii.i.ii, 
...
for(pid in unique(trials$pid)) {
  ...
  df.xii.ii.i <- rbind(df.xii.ii.i, 
                       data.frame(pid,
                                  AiCPref=mean(tmp$adviceType[tmp$type==trialTypes$choice]==adviceTypes$AiC),
                                  DotDiffCoef=lm(dotDifference ~ id, tmp)$coefficients[2]))
}
cor.test(df.xii.ii.i$AiCPref, df.xii.ii.i$DotDiffCoef)
correlationBF(df.xii.ii.i$AiCPref, df.xii.ii.i$DotDiffCoef)
```

```{r echo = F}
gg.xii.ii.i <- ggplot(df.xii.ii.i, aes(x = AiCPref, y = DotDiffCoef)) +
  geom_vline(linetype = "dashed", color = "black", xintercept = .5, size = 1) +
  geom_smooth(method = 'lm') +
  geom_point(aes(colour = as.factor(pid))) +
  labs(title = 'Correlation between dot difference increase and AiC preference') +
  style.long
gg.xii.ii.i
```
...
cor.test(df.xii.ii.ii$AiCPref, df.xii.ii.ii$Resolution)
correlationBF(df.xii.ii.ii$AiCPref, df.xii.ii.ii$Resolution)
```

```{r echo = F}
gg.xii.ii.ii <- ggplot(df.xii.ii.ii, aes(x = AiCPref, y = Resolution)) +
  geom_vline(linetype = "dashed", color = "black", xintercept = .5, size = 1) +
  geom_smooth(method = 'lm') +
  geom_point(aes(colour = as.factor(pid))) +
  labs(title = 'Correlation between metacognitive resolution and AiC preference') +
  style.long
gg.xii.ii.ii
```
...
for(pid in unique(participants$pid)) {
  for(early in c(T,F)) { # split by advisor sets 1 and 2 (blocks 2&3, 4&5)
    ...
    aid <- ifelse(mean(tmp$adviceType==adviceTypes$AiC), adviceTypes$AiC, adviceTypes$AiU)
...
We've tried many ways of analysing influence. Here we just quite simply t-test medium confidence trials. We look at correct trials for parity with Experiment 1.

```{r results='asis'}
tmp <- trials[trials$confidenceCategory == confidenceCategories$medium & trials$initialCorrect == T, ]
