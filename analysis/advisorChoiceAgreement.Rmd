...
printMean(tmp$adviceType, 'Mean HighAgr pick rate')
print('Choice proportion HighAgr vs. chance level (.5)')
prettyPrint(t.vi.i, d)
print('Bayesian examination of above (prior = mean of 0.5, sd as empirically observed)')
print(tB.vi.i)
print(paste0('Evidence strength for preferential HighAgr picking: BF=', round(exp(tB.vi.i@bayesFactor$bf),3)))
```
...
printMean(tmp$adviceType, 'Mean HighAgr pick rate')
print('Choice proportion HighAgr vs. chance level (.5) for mid-confidence trials')
prettyPrint(t.vi.ii,d)
print('Bayesian examination of above (prior = mean of 0.5, sd as empirically observed)')
print(tB.vi.ii)
print(paste0('Evidence strength for preferential HighAgr picking: BF=', round(exp(tB.vi.ii@bayesFactor$bf),3)))
```
...
print('Means:')
printMean(tmp$influence[tmp$adviceType==adviceTypes$HighAgr], 'Mean|HighAgr')
printMean(tmp$influence[tmp$adviceType==adviceTypes$LowAgr], 'Mean|LowAgr')
...
print('Means:')
printMean(tmp$rawInfluence[tmp$adviceType==adviceTypes$HighAgr], 'Mean|HighAgr')
printMean(tmp$rawInfluence[tmp$adviceType==adviceTypes$LowAgr], 'Mean|LowAgr')
...
for(pid in unique(participants$pid)) {
  ...
  df.xii.i.i <- rbind(df.xii.i.i, data.frame(pid,
                                             ...
                                             InfluenceDiff=(sum(tmp$influence[tmp$adviceType==adviceTypes$HighAgr]) -
                                                              sum(tmp$influence[tmp$adviceType==adviceTypes$LowAgr]))))
}
...
```{r echo = F}
gg.xii.i.i <- ggplot(df.xii.i.i, aes(x = HighAgrPref, y = InfluenceDiff)) +
