...
for(let run = 0; run < runs; run++) {
    ...
    describe('ACv2/dbc (run=' + run + ')', function () {

        it('Checks browser compatability', function () {
            cy.visit('localhost/ExploringSocialMetacognition/ACv2/dbc.html?PROLIFIC_PID=CypressTest');
        ...
        it('Creates a NoFeedbackContexts object', function () {
            cy.window()
                .its('study')
                .its('constructor')
                .its('name')
                .should('eq', 'NoFeedbackContexts');
        });
